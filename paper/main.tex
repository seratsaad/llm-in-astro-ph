...
\shorttitle{Most recent astronomy papers show language-model use}
\shortauthors{Saad \& Ting}

\begin{document}

\title{More than half of recent astronomy papers show
language-model use}
...
We pass the controls and the nulls through the identical pipeline at every stage. Any prevalence they generate does not measure language-model use. It measures our own systematic error, and we report it as such. We treat this as our main diagnostic, and in Section~\ref{sec:placebo} we show that the document-frequency estimator does not survive it at full-text lengths.
...
\subsection{Document frequency does not transfer to full text}\label{sec:placebo}
...
The mechanism is document length. The median analyzed body grew from about 4{,}500 tokens in 2015 to 6{,}400 in 2025, and the probability that a fixed-rate word appears at least once rises with length. A statistic that asks only whether a word appears therefore reads longer papers as more contaminated. When we measure occurrences per token instead, most of the control excess disappears, as the top row of Figure~\ref{fig:rates} shows. On the per-token scale the markers rise by a factor of \markerRiseFulltext\ by 2025 while the controls stay near flat, and the small drift they retain is the ordinary-vocabulary change that the background term of our model absorbs.

The within-section panels answer a second question. The unlabeled share of the body shrinks over the decade, so a whole-document rate mixes a change in composition into the measurement. Within each of the five named sections we find the marker rise at similar strength, and within each section we find the controls flat. The rise is therefore a property of the prose rather than of which parts of papers the OCR labels.
