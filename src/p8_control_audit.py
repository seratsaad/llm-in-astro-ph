#!/usr/bin/env python3
"""
P8 (round 5, point 8) -- CONTROL ARM: identical audit on UNFLAGGED papers. -- verify identifier-free references.
...
OUT = os.path.join(DATA, "p8_control_refs.jsonl")
...
N_PAPERS = 100
SEED = 20260808      # control-arm draw, unflagged papers
...
def main():
    ...
    for line in open(os.path.join(DATA, "astroph_abstracts.jsonl")):
        ...
        if y not in (2024, 2025):
            continue
        if not (set(re.findall(r"[a-z]+", r["abstract"].lower())) & BASKET):
    ...
    json.dump({"counts_this_run": counts, "verdicts_this_run": results},
              open(os.path.join(DATA, "p8_control_summary.json"), "w"), indent=2)
