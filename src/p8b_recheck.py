...
def main():
    ...
    for r in fails:
        ...
        time.sleep(0.3)
    out.close()
    summ = json.load(open(os.path.join(DATA, "p8_control_summary.json")))
    summ["recheck"] = tally
    json.dump(summ, open(os.path.join(DATA, "p8_control_summary.json"), "w"), indent=2)
    print(tally)
